program validate_coordinate_fix
  use ft_face_unified
  use ft_types
  use ft_outline_mod
  ...
  print *, "VALIDATION: Coordinate Transformation Fix"
  print *, "========================================"
  print *, ""
  ...
  print *, "Original outline coordinates (font units):"
  print *, "  X range: [", outline%points(1)%x, " to ??? ]"
  print *, "  Y range: [", outline%points(1)%y, " to ??? ]"
  ...
  print *, "Transformed outline coordinates (bitmap pixels):"
  print *, "  X range: [", transformed_outline%points(1)%x, " to ??? ]"
  print *, "  Y range: [", transformed_outline%points(1)%y, " to ??? ]"
  print *, ""
  ...
  if (non_zero_pixels > 0) then
    print *, "  ✓ SUCCESS: Pixels are being rendered!"
    print *, "  ✓ Coordinate transformation is working!"
  else
    print *, "  ✗ FAILURE: No pixels rendered"
    print *, "  ✗ Coordinate transformation needs debugging"
  end if
...
end program validate_coordinate_fix